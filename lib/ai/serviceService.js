...
export async function findNearbyServices(serviceType, latitude, longitude, minRating, location = {}) {
  ...
  return services
    .map(service => {
      ...
      const hasProviderCoordinates = service.latitude != null && service.longitude != null;
      const requestedAreaDistance = hasProviderCoordinates && requestedLocationCenter
        ? calculateDistance(requestedLocationCenter.latitude, requestedLocationCenter.longitude, service.latitude, service.longitude)
        : null;
      const userDistance = hasProviderCoordinates && hasUserCoordinates
