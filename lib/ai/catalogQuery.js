const SERVICE_ALIASES = {
  ...
  food: ['food vendor', 'caterer', 'catering', 'food service'],
  carpenter: ['carpenter', 'carpentry', 'furniture repair'],
  plumbing: ['plumber', 'plumbing'],
  cleaning: ['cleaner', 'cleaning service', 'cleaning'],
...
function extractServiceLocation(message) {
  if (/\b(around me|near me|nearby|closest)\b/i.test(message)) {
    return { locationMode: 'nearby' };
  }

  const locationMatch = message.match(/\b(?:in|at)\s+([a-z][a-z\s'-]*?)(?=\s*(?:[,.!?]|$))/i);
  const locationQuery = locationMatch?.[1]?.trim();

  if (locationQuery && !['home', 'my area', 'my location'].includes(locationQuery)) {
    return { locationMode: 'explicit', locationQuery };
  }

  return { locationMode: 'any' };
}

function createServiceQuery(serviceType, message) {
  return {
    intent: 'find_service',
    service_type: serviceType,
    ...extractServiceLocation(message),
  };
}
...
export function extractCatalogQuery(message) {
  ...
  if (/\b(fix|repair|service)\b.*\b(car|vehicle)\b|\b(car|vehicle)\b.*\b(fix|repair|service)\b/i.test(normalized)) {
    return createServiceQuery('mechanic', normalized);
  }

  for (const [serviceType, aliases] of Object.entries(SERVICE_ALIASES)) {
    if (aliases.some(alias => normalized.includes(alias))) {
      return createServiceQuery(serviceType, normalized);
    }
  }
