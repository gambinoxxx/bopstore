      ...
      },
      {
        ...
        parameters: {
          ...
          properties: {
            ...
            date_time: { type: "STRING", description: "The date and time for the appointment. Convert user inputs (e.g., '20/06', 'tomorrow') into a valid ISO 8601 date string (YYYY-MM-DD)." },
            phone: { type: "STRING", description: "The user's phone number if mentioned." },
            whatsapp: { type: "STRING", description: "The user's WhatsApp number if mentioned." },
            notes: { type: "STRING", description: "Any special requests or notes from the user." },
          },
...
3. ${location?.latitude != null ? `Current User Location: Latitude ${location.latitude}, Longitude ${location.longitude}.` : "User location is currently unknown."}
4. CRITICAL: NEVER ask the user for their numeric 'latitude' or 'longitude'. This is too technical and bad for business.
5. If location is missing but needed for a service, call 'find_service' anyway without coordinates, or politely ask the user to 'allow location access' in their browser.
6. If location is provided in these instructions (point 3), use it automatically for tool calls.
7. Use tools immediately for actions. For general talk, be natural and warm.` }],
      },
    });
